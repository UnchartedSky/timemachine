...
function setPostsPrivate(request, response) {
    "use strict";

    const accessToken = request.body.accessToken;
    const limit = request.body.limit ? request.body.limit : 128;

    const isTrial = request.body.isTrial;
    const since = isTrial ? timestamp.now('-1M') : request.body.since;
    const until = request.body.until ? request.body.until : undefined;

    var fb_options = {'limit': limit};
    if(!util.isNullOrUndefined(since)) {
        fb_options.since = since;
    }

    if(!util.isNullOrUndefined(until)) {
        fb_options.until = until;
    }

    iteratePosts(accessToken, fb_options, response, function (post) {

        winston.info('This post is going to be set to only me: ' + JSON.stringify(post));

        var fb_options = {'privacy': {'value': 'SELF'}};
        fbApp.api(post.id, 'post', fb_options, function (res) {
            if (!res || res.error) {
                var errorMsg = !res ? 'error occurred' : res.error;
                winston.error(errorMsg);
                response
...
module.exports.setAllPostsPrivate = setPostsPrivate;